...
Public Class Frm_Inv_PURAr
    Private Sub Frm_Inv_PURAr_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        FillInv_Spplier()
        FillGen_Branchs()
        FillAcc_Paments()
        Label2.Text = Now
        ...
        Dim da As New SqlClient.SqlDataAdapter("SELECT    isnull(max(PUM_Id),0) + 1 as PUM_Id     FROM          TbL_Inv_PurchasesMaster", con)
    ...
    Private Sub DataGridView1_CellEndEdit(sender As Object, e As DataGridViewCellEventArgs) Handles DataGridView1.CellEndEdit
        Try
            ...
            For i As Integer = 0 To DataGridView1.Rows.Count - 2
                ...
                DataGridView1.Rows(i).Cells("total").Value = Double.Parse(DataGridView1.Rows(i).Cells("qty").Value) * Double.Parse(DataGridView1.Rows(i).Cells("Item_Cost").Value)
    ...
    Private Sub RadButton5_Click(sender As Object, e As EventArgs) Handles RadButton5.Click
        If RadTextBox2.Text = String.Empty Then
            MsgBox("الرجاء ادخال رقم الفاتوره")
            RadTextBox2.Focus()
        ...
        ElseIf RadTextBox3.Text = 0 Then
            ...
            MsgBox("لا يسمح بحفظ الفاتوره والقيمه صفرية")
            RadTextBox3.Focus()
        ...
        Else
            ...
            makeconnection()
            Dim da1 As New SqlClient.SqlDataAdapter("SELECT    isnull(max(PUM_Id),0) + 1 as PUM_Id     FROM          TbL_Inv_PurchasesMaster", con)
            Dim dt1 As New DataTable
            da1.Fill(dt1)
            RadTextBox1.Text = dt1.Rows(0).Item("PUM_Id")
            Dim da As New SqlClient.SqlDataAdapter("INSERT INTO TbL_Inv_PurchasesMaster                           (PUM_Id, SUP_Id, PUM_Date, PUM_InvoceNo, PUM_Note, PUM_SubTotal, PUM_DesAmount, PUM_Total, PUM_TaxAmount, PUM_Post, Pament_id, user_id, AddDate, Branch_Id)  VALUES        (N'" & RadTextBox1.Text & "', N'" & ComboBox1.SelectedIndex + 1 & "', N'" & RadDateTimePicker1.Text & "', N'" & RadTextBox2.Text & "', N'" & RadTextBox9.Text & "', N'" & RadTextBox3.Text & "', N'" & RadTextBox4.Text & "',                           N'" & RadTextBox6.Text & "', N'" & RadTextBox5.Text & "', N'0', N'" & ComboBox3.SelectedIndex + 1 & "', N'" & Label1.Text & "', N'" & Label2.Text & "', N'" & ComboBox2.SelectedIndex + 1 & "')  ", con)
            Dim dt As New DataTable
            da.Fill(dt)


            For i = 0 To DataGridView1.Rows.Count - 2
                Dim da2 As New SqlClient.SqlDataAdapter("INSERT        INTO              TBL_inv_PurchasesDetiles(PUM_ID, PUD_Barcode, PUD_ItemID, PUD_Name, PUD_QTY, PUD_BQTY, PUD_Cost, PUD_Total, UN_ID) VALUES        (N'" & RadTextBox1.Text & "', N'" & DataGridView1.Rows(i).Cells("Item_BarCode").Value & "', N'0', N'" & DataGridView1.Rows(i).Cells("Item_NameAr").Value & "',                           N'" & DataGridView1.Rows(i).Cells("QTY").Value & "', N'" & DataGridView1.Rows(i).Cells("BQTY").Value & "', N'" & DataGridView1.Rows(i).Cells("Item_Cost").Value & "', N'" & DataGridView1.Rows(i).Cells("Total").Value & "',                           N'" & DataGridView1.Rows(i).Cells("Unit_id").Value & "') ", con)
                Dim dt2 As New DataTable
                da2.Fill(dt2)


                Dim da3 As New SqlClient.SqlDataAdapter("INSERT         INTO              TBL_Inv_Batches(B_Barcode, B_Name, B_INQTY, B_OQTY, B_Balance, B_Source, Item_id)  VALUES        (N'" & DataGridView1.Rows(i).Cells("Item_BarCode").Value & "', N'" & DataGridView1.Rows(i).Cells("Item_NameAr").Value & "', N'" & DataGridView1.Rows(i).Cells("SumQty").Value & "', 0, 0, N'مشتريات', N'0')  ", con)
                Dim dt3 As New DataTable
                da3.Fill(dt3)


                Dim da4 As New SqlClient.SqlDataAdapter("UPDATE       Tbl_Inv_Items  SET                Item_Cost = N'" & DataGridView1.Rows(i).Cells("Item_Cost").Value & "'  WHERE        (Item_BarCode = N'" & DataGridView1.Rows(i).Cells("Item_BarCode").Value & "')", con)
